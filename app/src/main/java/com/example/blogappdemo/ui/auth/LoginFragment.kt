...
import com.example.blogappdemo.databinding.FragmentLoginBinding
...
class LoginFragment : Fragment(R.layout.fragment_login) {
    ...
    private val firebaseAuth by lazy { FirebaseAuth.getInstance() } //el by lazy nos permite inicializarlo cuadno lo usamos y no siempre para ahorra "memoria", es decir para no tenerla activa siempre si no cuando la usemos
    ...
    private fun isUserLoggedIn(){
        //El current si nos devuelve nulo significa que el usuario no existe
    ...
    private fun signIn(email: String,password: String){
        viewModel.singIn(email, password).observe(viewLifecycleOwner, Observer {
            when(it){
                is Result.Loading ->{
                ...
                is Result.Success ->{
                    ...
                    if(it.data?.displayName.isNullOrEmpty()){
                        findNavController().navigate(R.id.action_loginFragment_to_setupProfileFragment)
                        Toast.makeText(requireContext(),"display: ${it.data?.displayName}",Toast.LENGTH_LONG).show()
